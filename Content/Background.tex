...
Unfortunately,~\cref{eq:loss_ssl_sim} has a fatal flaw: its minimizer is a network $f_{\theta^*}: \mathbb{R}^{C\times H\times W}\rightarrow[0]^d$. 
...
% HERE SIMSIAM AND DINO ONCE I HAVE INTERNET TO WRITE IT AS GOOD AS IT GETS.

\subsection{Dissimilar Images and Contrastive Learning}

While \textit{similar} images' embeddings should stay close, prior work~\cite{radford2021learning,chen2020simple} found it important that \textit{dissimilar} images' embeddings stay far from each other. Analogously to~\cref{sec:similar_imgs_ssl}, it is not clear what does exactly mean that two images are dissimilar. 

The trivial solution is to assume that two different images are dissimilar (from the fact that they are \textit{different}), which is exactly what Contrastive Learning does. InfoNCE training loss defined as:

\begin{equation}
    \label{eq:infonce}
    \text{InfoNCE}=-\log\frac{\exp(-\text{dist}(z_i,z_j)/\tau)}{}
\end{equation}
